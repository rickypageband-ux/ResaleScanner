...
import com.resalescanner.app.ui.ProductSearchState
...
@Composable
private fun ResultContent(viewModel: AppViewModel, query: String, result: ProductSearchResult, onAdded: () -> Unit) {
    ...
    Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(18.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
        ...
        Surface(color = MaterialTheme.colorScheme.primaryContainer, shape = MaterialTheme.shapes.small) { Text(result.providerMessage ?: "Live eBay pricing", Modifier.padding(10.dp), style = MaterialTheme.typography.labelMedium) }
        Text("Marketplace Prices", style = MaterialTheme.typography.titleLarge)
        ...
        Text("eBay sold-listing data requires separate eBay approval. Current figures use active listings.", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
        ...
        if (purchasePrice.isNotBlank()) Card(Modifier.fillMaxWidth()) { Column(Modifier.padding(18.dp)) { Text("Estimated total profit"); Text(money(profitCents), style = MaterialTheme.typography.headlineMedium, color = if (profitCents >= 0) MaterialTheme.colorScheme.primary else MaterialTheme.colorScheme.error); Text("For ${quantityValue ?: 1} item(s), before fees and shipping", style = MaterialTheme.typography.bodySmall) } }
        Button({ viewModel.save(ItemDraft(barcode = query.takeIf { it.length in 8..14 && it.all(Char::isDigit) }.orEmpty(), title = result.title, description = "Added from live eBay product search: $query", purchasePrice = purchasePrice, askingPrice = String.format(Locale.US, "%.2f", result.suggestedResalePriceCents / 100.0), quantity = quantity), onAdded) }, enabled = canAdd, modifier = Modifier.fillMaxWidth()) { Text("Add to My Inventory") }
        ...
        Text("Amazon: not connected  •  Walmart: not connected", style = MaterialTheme.typography.labelMedium, color = MaterialTheme.colorScheme.onSurfaceVariant)
    }
}

@Composable private fun PriceRow(label: String, cents: Long, emphasized: Boolean = false) { Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) { Text(label, style = if (emphasized) MaterialTheme.typography.titleMedium else MaterialTheme.typography.bodyLarge); Text(money(cents), style = if (emphasized) MaterialTheme.typography.titleMedium else MaterialTheme.typography.bodyLarge, color = if (emphasized) MaterialTheme.colorScheme.primary else MaterialTheme.colorScheme.onSurface) } }
