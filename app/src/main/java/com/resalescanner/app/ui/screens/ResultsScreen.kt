...
fun ResultsScreen(query: String, onBack: () -> Unit) {
    ...
    val profitCents = result.suggestedResalePriceCents - purchaseCents
    Scaffold(topBar = { TopAppBar(title = { Text("Results") }, navigationIcon = { IconButton(onBack) { Icon(Icons.AutoMirrored.Outlined.ArrowBack, "Back") } }) }) { padding ->
        Column(Modifier.fillMaxSize().padding(padding).verticalScroll(rememberScrollState()).padding(18.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
            ...
            OutlinedTextField(purchasePrice, { purchasePrice = it }, label = { Text("Your purchase price") }, prefix = { Text("$") }, singleLine = true, keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Decimal), modifier = Modifier.fillMaxWidth())
            if (purchasePrice.isNotBlank()) Card(Modifier.fillMaxWidth()) { Column(Modifier.padding(18.dp)) { Text("Estimated profit"); Text(money(profitCents), style = MaterialTheme.typography.headlineMedium, color = if (profitCents >= 0) MaterialTheme.colorScheme.primary else MaterialTheme.colorScheme.error); Text("Before marketplace fees and shipping", style = MaterialTheme.typography.bodySmall) } }
        }
    }
}
